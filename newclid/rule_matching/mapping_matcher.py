    ...
    def _match_generic(self, rule: Rule, proof: ProofState) -> set[Justification]:
        ...
        for mapping in self.theorem_mapper.mappings(rule, points, proof=proof):
            ...
            for premise in rule.premises:
                premise_args = tuple(mapping.get(arg, arg) for arg in premise.variables)
            ...
            for conclusion in rule.conclusions:
                conclusion_args = tuple(mapping.get(arg, arg) for arg in conclusion.variables)
...
class FilterMapper(TheoremMapper):
    def mappings(
        ...
        for premise in symbolic_premises:
            # print(f"Matching premise {premise.name} for rule {rule.id}")
    ...
    def _initialize_valid_arg_permutations(
        ...
        for predicate in self.predicates:
            ...
            for stmt_permutation in points_iterator:
                if not isinstance(stmt_permutation, EquationStatement):
                ...
                if isinstance(stmt_permutation, EquationStatement):
                    # For equations, matching is complex. For now, we skip indexing variables.
                    continue
