...
def predicate_from_construction(
    ...
    match construction.predicate_type:
        ...
        case PredicateType.ANGLE_EQUATION | PredicateType.LENGTH_EQUATION:
            new_args = []
            for arg in canonical_args:
                if arg in points_registry.name_to_point:
                    new_args.append(points_registry.name_to_point[arg])
                else:
                    new_args.append(arg)
            if construction.predicate_type == PredicateType.ANGLE_EQUATION:
                return AngleEquation(args=tuple(new_args))
            return LengthEquation(args=tuple(new_args))
